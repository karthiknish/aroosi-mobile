...
export const NavGuideProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Start guide once all targets known & user new
  useEffect(() => {
    ...
    <NavGuideContext.Provider value={{ registerTarget, registerTabBar }}>
      ...
      {show && currentTarget && tabBarLayout && (
        <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
          <View style={[styles.overlay, { backgroundColor: rgbaHex(theme.colors.text.primary, 0.55) }]}>
            {/* Highlight */}
            <View
              style={[
                styles.highlight,
                {
                  ...
                  top: tabBarLayout.y + currentTarget.y + (Platform.OS === "android" ? -8 : -8),
            ...
            >
              ...
              <View style={styles.tooltipButtons}>
                <Pressable onPress={finish} style={[styles.secondaryBtn, { borderColor: theme.colors.border.primary }]}> 
              ...
              <Text style={{ marginTop: 6, fontSize: 11, color: theme.colors.text.tertiary, textAlign: "center" }}>
                {step + 1} / {STEPS.length}
              </Text>
            </View>
          </View>
