...
export default function QuickPicksScreen() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!res.success)
        throw new Error(res?.error?.message || "Failed to load quick picks");
  ...
  const dailyLimit = useMemo(
    () => (plan === "premiumPlus" ? 40 : plan === "premium" ? 20 : 5),
    [plan]
  );
  ...
  const ordered: QuickPickProfile[] = useMemo(() => {
    ...
    const coalesceViews = (p: any) =>
      Number(
        p?.viewsToday ??
          p?.profileViewsToday ??
          (typeof p?.views === "object" ? p?.views?.today : p?.views) ??
          0
      ) || 0;
  ...
  const onAction = useCallback(
    async (action: "like" | "skip") => {
      ...
      if (action === "like") {
        try {
          const avail = await canUseFeatureNow("likesSent");
          if (!avail.canUse) {
            // Surface a gentle hint; SwipeableCard overlay remains disabled
            return;
          }
        } catch {}
      }
      ...
      if (action === "like") {
        try {
          await trackFeatureUsage("likesSent");
        } catch {}
      }
    },
    [api, index, userIds, canUseFeatureNow, trackFeatureUsage]
  );

  const buildProfileForCard = (p?: QuickPickProfile) => {
    if (!p) return null;
    const topIceQs = Array.isArray(iceQs)
      ? iceQs
          .slice(0, 2)
          .map((q: any) => q.text)
          .filter(Boolean)
    ...
    return (
      <ScreenContainer>
        <View style={{ padding: Layout.spacing.lg }}>
          <ApiErrorDisplay
            error={new Error(loadError || "Error")}
            onRetry={refetch}
          />
        </View>
      </ScreenContainer>
  ...
  return (
    <ErrorBoundary>
      ...
      >
        <AppHeader
          title="Daily Quick Picks"
          subtitle={`${Math.min(index + 1, userIds.length)} / ${
            userIds.length
          } shown • Limit ${dailyLimit} (${plan})`}
        />
        {(() => {
          const f = usage?.features?.find((x: any) => x.name === "likesSent");
          const reached = (f?.percentageUsed ?? 0) >= 100;
          if (reached) {
            return (
              <View
                style={{ paddingHorizontal: Layout.spacing.lg, marginTop: 6 }}
              >
                <HintPopover
                  label="Why?"
                  hint={
                    "You've reached today’s like limit. Upgrade for unlimited or higher daily likes."
                  }
                />
              </View>
            );
        ...
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("ProfileTab", { screen: "Icebreakers" })
          }
          style={{
        ...
        >
          ...
          >
            Answer today’s icebreakers
            {iceTotal > 0 ? ` (${iceAnswered}/${iceTotal})` : ""}
          </Text>
        </TouchableOpacity>
        {loadError && (
          <View
          ...
          >
            <Text style={{ color: theme.colors.error[700], fontSize: 12 }}>
              {loadError}
            </Text>
          </View>
        )}
