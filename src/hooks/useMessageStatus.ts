...
// Local status type aligned with message.ts optional status field plus internal transient 'pending' state
...
export interface UseMessageStatusResult {
  ...
  sendDeliveryReceipt: (messageId: string) => void;
  sendReadReceipt: (messageId: string) => void;
  markMessagesAsRead: (messageIds: string[]) => void;
  handleMessageReceived: (message: Message) => void;
  handleMessageSent: (messageId: string) => void;
  ...
  getReadTimestamp: (messageId: string) => number | undefined;
  getDeliveryTimestamp: (messageId: string) => number | undefined;
  getUnreadMessages: (messages: Message[]) => Message[];
...
export function useMessageStatus({
  ...
  const sendDeliveryReceipt = useCallback(
    (messageId: string) => {
      if (realtimeService) {
        // Signature: (messageId, conversationId, status)
  ...
  const sendReadReceipt = useCallback(
    (messageId: string) => {
      if (realtimeService) {
  realtimeService.sendDeliveryReceipt(messageId, conversationId, "read");
