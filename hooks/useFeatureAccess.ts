    ...
    async (feature: keyof FeatureLimits): Promise<FeatureAccessResult> => {
      try {
        ...
        if (
          ...
          if (featureName) {
            const usageStatus = await canUseFeatureNow(featureName);
  ...
  const validateAndExecute = useCallback(
    async <T>(
      ...
      try {
        ...
        if (featureName) {
          try {
            await trackFeatureUsage(featureName as keyof FeatureUsage);
          } catch (trackingError) {
...
function getFeatureUpgradeMessage(feature: keyof FeatureLimits): string {
  const messages: Record<string, string> = {
    ...
    canBoostProfile: "Upgrade to Premium Plus to boost your profile visibility",
