...
export interface AuthContextType {
  ...
  authFetch: (input: RequestInfo | URL, init?: RequestInit & { autoRefreshToken?: boolean }) => Promise<Response>;
  signIn: (email: string, password: string) => Promise<{ success: boolean; error?: string }>;
  signUp: (email: string, password: string, fullName: string) => Promise<{ success: boolean; error?: string; emailVerificationSent?: boolean; emailVerified?: boolean }>;
  verifyEmailCode: () => Promise<{ success: boolean; error?: string }>; // now just reloads user to detect verification
  signInWithGoogle: (credential?: string, state?: string) => Promise<{ success: boolean; error?: string }>;
  signOut: () => void;
  ...
  startEmailVerificationPolling?: (opts?: { intervalMs?: number; maxAttempts?: number }) => void;
}
...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const fetchUser = useCallback(async (): Promise<User | null> => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/auth/me`, { headers: { Authorization: `Bearer ${idToken}` } });
      ...
      if (process.env.NODE_ENV !== 'production') console.warn('AuthProvider.fetchUser failed', e);
      return null;
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onChange = (next: AppStateStatus) => {
      if (currentState.match(/inactive|background/) && next === 'active') refreshUser();
      currentState = next;
    };
  ...
  const signUp = useCallback(async (email: string, password: string, fullName: string) => {
    try {
      ...
      const current = getFirebaseAuth().currentUser;
      if (current) {
        try { await firebaseUpdateProfile(current, { displayName: fullName }); } catch {}
        try { await sendEmailVerification(current); } catch {}
      }
      await refreshUser();
  return { success: true, emailVerificationSent: true, emailVerified: !!current?.emailVerified, needsEmailVerification: !current?.emailVerified } as any;
    } catch (e: any) {
  ...
  const startEmailVerificationPolling = useCallback((opts?: { intervalMs?: number; maxAttempts?: number }) => {
    ...
    emailPollIntervalRef.current = setInterval(async () => {
      const current = getFirebaseAuth().currentUser;
      if (!current) {
        if (emailPollIntervalRef.current) { clearInterval(emailPollIntervalRef.current); emailPollIntervalRef.current = null; }
        return;
      }
      try {
        await current.reload();
  ...
  useEffect(() => {
    return () => { if (emailPollIntervalRef.current) clearInterval(emailPollIntervalRef.current); };
  }, []);
  ...
  const sendPasswordReset = useCallback(async (email: string) => {
    try {
      ...
      else if (e?.code === 'auth/user-not-found') return { success: true }; // avoid leaking existence
      else if (e?.code === 'auth/too-many-requests') msg = 'Too many requests. Try later';
      return { success: false, error: msg };
    }
  }, []);

  const resetPasswordWithCode = useCallback(async (code: string, newPassword: string) => {
    try {
      ...
      if (!(hasLower && hasUpper && hasDigit && hasSymbol)) {
        return { success: false, error: 'Password must include upper, lower, number, symbol.' };
      }
      ...
      else if (e?.code === 'auth/invalid-action-code') msg = 'Invalid reset code';
  ...
  const signInWithGoogle = useCallback(async () => {
    try {
      const clientId = process.env.EXPO_PUBLIC_GOOGLE_OAUTH_CLIENT_ID || process.env.GOOGLE_OAUTH_CLIENT_ID;
      if (!clientId) return { success: false, error: 'Missing Google client id' };
      ...
      const result: any = await (AuthSession as any).startAsync({ authUrl });
      if (result.type !== 'success' || !result.params.access_token) {
        return { success: false, error: 'Google auth canceled' };
      }
      // Exchange Google access token for Firebase credential
      const credential = GoogleAuthProvider.credential(undefined, result.params.access_token as string);
  ...
  const signOut = useCallback(async () => {
    ...
    try { await firebaseSignOut(getFirebaseAuth()); } catch (e) { console.error('Sign out failed', e); } finally { setUser(null); setError(null); }
  }, []);
  ...
  const value: AuthContextType = {
    ...
    getToken: async () => (await getFirebaseAuth().currentUser?.getIdToken()) || null,
