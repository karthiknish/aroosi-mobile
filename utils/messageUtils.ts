...
export function shouldShowTimestamp(
  ...
  const currentSender = currentMessage.fromUserId;
  const previousSender = previousMessage.fromUserId;
...
export function isMessageRead(
  ...
  const sender = message.fromUserId;
...
export function isVoiceMessage(message: Message): boolean {
  return (
    message.type === "voice" && !!(message.audioStorageId || message.voiceUrl)
  );
}
...
export function normalizeConversation(conversation: any): Conversation {
  return {
    ...
    lastMessage: conversation.lastMessage
      ...
      : undefined,
    lastActivity:
      conversation.lastActivity || conversation.lastMessageAt || Date.now(),
    ...
    updatedAt:
      conversation.updatedAt || conversation.lastActivity || Date.now(),
  };
}
